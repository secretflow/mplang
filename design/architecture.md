...
This document describes the MPLang architecture, a framework designed for
multi-party secure computation with clear separation of concerns.

Key design goals:

- **Explicit State Management:** All cluster configuration and execution context
  is explicitly managed, making behavior predictable and testable.

- **Clean Abstractions:** High-level device-oriented APIs hide implementation
  details like `rank`, while low-level party-oriented APIs provide full control
  when needed.

- **Clear Role Separation:** The architecture distinguishes between physical
  compute nodes and logical computation domains, enabling complex scenarios
  like TEE integration.

The architecture achieves clarity and scalability through strict separation
between physical and logical concepts.

## 2. Core Philosophy: Physical vs. Logical Worlds

The architecture is built upon a fundamental duality:
...
To reflect the PP/VP duality, the cluster configuration consists of two
...
The MPLang architecture provides a robust, pragmatic foundation for multi-party
secure computation. The structure achieves:
...
- **Extensibility**: Easy to add new device types and protocols without architectural changes
...
5. **Future-Proof**: Extensible design supports new backends and protocols
