...
def test_spu_config_from_dict_reads_flat_profiling_flags(enabled_field):
    config = spu.SPUConfig.from_dict({enabled_field: True})
    ...
    for field_name in (
        ...
        assert getattr(config, field_name) is (field_name == enabled_field)
