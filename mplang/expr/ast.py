    ...
    def _compute_mptypes(self) -> list[MPType]:
        ...
        if self.rmask is not None:
            ...
            if deduced_pmask is not None:
                ...
                rmask_obj = self.rmask if isinstance(self.rmask, Mask) else Mask(self.rmask)
                deduced_pmask_obj = deduced_pmask if isinstance(deduced_pmask, Mask) else Mask(deduced_pmask)
                if not rmask_obj.is_subset_of(deduced_pmask_obj):
                    raise ValueError(
...
class ConvExpr(Expr):
    ...
    def _compute_mptypes(self) -> list[MPType]:
        ...
        mask_objects = [pmask if isinstance(pmask, Mask) else Mask(pmask) for pmask in non_none_pmasks]
        
        if not Mask.are_disjoint(*mask_objects):
        ...
        if dynamic_pmask:
            ...
            out_pmask = Mask.union(*mask_objects)
...
class ShflSExpr(Expr):
    ...
    def __init__(self, src_val: Expr, pmask: Mask, src_ranks: list[Rank]):
        ...
        pmask_obj = self.pmask if isinstance(self.pmask, Mask) else Mask(self.pmask)
        if len(self.src_ranks) != pmask_obj.bit_count():
            raise ValueError(
        ...
        for i, rank in enumerate(self.src_ranks):
            ...
            if src_pmask is not None:
                src_pmask_obj = src_pmask if isinstance(src_pmask, Mask) else Mask(src_pmask)
                if not src_pmask_obj.contains_rank(rank):
                    raise ValueError(
