...
class SPU(SecureAPI):
    ...
    def seal(self, obj: MPObject, frm_mask: Mask | None = None) -> list[MPObject]:
        ...
        if obj.pmask is None:
            ...
            if frm_mask is None:
                ...
                # Ensure frm_mask is a Mask object
                if not isinstance(frm_mask, Mask):
                    frm_mask = Mask(frm_mask)
                # Ensure obj.pmask is a Mask object for comparison
                obj_pmask = obj.pmask if isinstance(obj.pmask, Mask) else Mask(obj.pmask) if obj.pmask is not None else None
                if obj_pmask is not None and not frm_mask.is_subset_of(obj_pmask):
        ...
        spu = SpuFE(world_size=spu_mask.bit_count())
        ...
        frm_mask = frm_mask if isinstance(frm_mask, Mask) else Mask(frm_mask)
        return [
            mpi.scatter_m(spu_mask, rank, shares)
            for rank in frm_mask.enum()
        ]
    ...
    def reveal(self, obj: MPObject, to_mask: Mask) -> MPObject:
        ...
        # (n_parties, n_shares)
        shares = [
            mpi.bcast_m(to_mask, rank, obj) for rank in spu_mask.enum()
        ]
        assert len(shares) == spu_mask.bit_count(), (shares, spu_mask)
