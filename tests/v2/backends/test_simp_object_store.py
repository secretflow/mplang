...
def test_simulator_object_store_flow():
    ...
    # 5. Verify fetch via client context
    fetched_0 = sim.client_ctx.fetch(0, uri_x0).result()
    fetched_1 = sim.client_ctx.fetch(1, uri_x1).result()
...
def test_simulator_fetch_mixed_values():
    ...
    res0 = sim.client_ctx.fetch(0, uri_0).result()
    assert res0 == 100
    assert val_1 == 200
