...
void solve_okvs(uint64_t *keys, uint64_t *values, uint64_t *output, uint64_t n,
                uint64_t m, uint64_t *seed_ptr);
...
inline Indices get_bin_local_indices(uint64_t key, uint64_t m_local,
                                     __m128i seed) {
...
bool solve_bin(const std::vector<uint64_t> &keys,
               const std::vector<__m128i> &vals, __m128i *P_local, uint64_t m,
               __m128i seed) {
...
void solve_okvs_opt(uint64_t *keys, uint64_t *values, uint64_t *output,
                    uint64_t n, uint64_t m, uint64_t *seed_ptr) {
  __m128i seed = _mm_loadu_si128((__m128i *)seed_ptr);

  // 1. Calculate Bin Boundaries
  // We divide M evenly among bins. The remainder is distributed to the first
  // few bins.
  ...
  // While Two-Choice Hashing (selecting the lighter of 2 potential bins) would
  // significantly reduce max bin load variance, it introduces "Statefulness".
  // The bin assignment for Key K would depend on the load of bins, which
  // depends on other keys. In standard PSI protocols (like RR22), the Decode
  // step must be capable of processing keys independently or without knowledge
  // of the full set distribution (Sender/Receiver separation). Therefore, we
  // use **Simple Binning** (Stateless Hash) where Bin(K) = H(K) % Bins. We
  // mitigate the resulting variance ("Balls-in-Bins" problem) by using a
  // slightly larger expansion factor (epsilon ~ 1.35) which is
  // bandwidth-acceptable and ensures stability.
  ...
  // 3. Parallel Solve
  // Each bin is solved independently. This logic is perfectly parallelizable
  // (embarrassingly parallel). The working set for each bin (~1000 items) stays
  // hot in L1 Cache.
...
#pragma omp critical
      {
        fprintf(stderr,
                "[WARN] Bin %lu failed optimized peeling; falling back to safe "
                "solver. Items: %lu / M: %lu (Ratio: %.2f)\n",
                b, bin_keys[b].size(), valid_m,
                (double)valid_m / bin_keys[b].size());

        // Prepare pointers for the safe solver
        uint64_t *keys_ptr = bin_keys[b].empty() ? nullptr : &bin_keys[b][0];
        uint64_t *vals_ptr =
            bin_vals[b].empty() ? nullptr : (uint64_t *)&bin_vals[b][0];
        uint64_t *out_ptr =
            output + (offset * 2ULL); // each 128-bit slot == 2 uint64_t

        // Call the safe solver implemented in okvs.cpp
        solve_okvs(keys_ptr, vals_ptr, out_ptr, bin_keys[b].size(), valid_m,
                   seed_ptr);
      }
    }
  }
...
void decode_okvs_opt(uint64_t *keys, uint64_t *storage, uint64_t *output,
                     uint64_t n, uint64_t m, uint64_t *seed_ptr) {
  __m128i seed = _mm_loadu_si128((__m128i *)seed_ptr);
...
#pragma omp parallel for schedule(static)
  for (uint64_t i = 0; i < n; ++i) {
    ...
    __m128i val =
        _mm_xor_si128(_mm_xor_si128(_mm_loadu_si128(&P_vec[offset + idx.h1]),
                                    _mm_loadu_si128(&P_vec[offset + idx.h2])),
