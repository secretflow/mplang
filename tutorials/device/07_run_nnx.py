...
- Stateful training: Batch normalization with running statistics
...
def extract_training_results(mp_result, simulator, owner_party="node_0"):
    """Extract training results from MPLang computation result.

    Args:
        mp_result: Result from mp.evaluate() containing training outputs
        simulator: MPLang simulator instance for fetching results
        owner_party: Party that owns the result data (unused, kept for API compatibility)

    Returns:
        Dictionary with loss_history, final_model_state, final_opt_state
        or None if extraction fails
    """
    try:
        # Fetch the result from the distributed computation
        raw_result = mp.fetch(simulator, mp_result)

        # MPLang result structure: (args_tree, kwargs_tree, (outputs, states))
        if isinstance(raw_result, tuple) and len(raw_result) == 3:
            _, _, outputs_tuple = raw_result

            if isinstance(outputs_tuple, tuple) and len(outputs_tuple) >= 3:
                output_list, final_model_state, final_opt_state = outputs_tuple

                # Find the loss history array (non-None element in the list)
                loss_history = None
        ...
        print(f"Unable to extract results from structure: {type(raw_result)}")
        return None
...
def extract_stateful_training_results(mp_result, simulator, owner_party="node_0"):
    """Extract training results from stateful model computation.

    Args:
        mp_result: Result from mp.evaluate() containing stateful training outputs
        simulator: MPLang simulator instance for fetching results
        owner_party: Party that owns the result data (unused, kept for API compatibility)

    Returns:
        Dictionary with loss_history, final_model_state, final_opt_state, bn_stats_history
        or None if extraction fails
    """
    try:
        # Fetch the result from the distributed computation
        ...
        if isinstance(raw_result, tuple) and len(raw_result) == 3:
            ...
            if isinstance(outputs_tuple, tuple) and len(outputs_tuple) >= 4:
                output_list, final_model_state, final_opt_state, bn_stats_history = (
                    outputs_tuple
                )
                ...
                if loss_history is not None and bn_stats_history is not None:
                    return {
                        "loss_history": loss_history,
                        "final_model_state": final_model_state,
                        "final_opt_state": final_opt_state,
                        "bn_stats_history": bn_stats_history,
                    }

        print(f"Unable to extract stateful results from structure: {type(raw_result)}")
        return None

    except Exception as e:
        print(f"Error extracting stateful training results: {e}")
        return None
...
cluster_config = mp.ClusterSpec.from_dict(
    {
        "nodes": [
        ...
        "devices": {
            "SP0": {  # SPU required by simulator
                "kind": "SPU",
                "members": ["node_0", "node_1", "node_2"],
                "config": {"protocol": "SEMI2K", "field": "FM128"},
            },
            "P0": {"kind": "PPU", "members": ["node_0"]},  # Plaintext Processing Unit
        },
    }
)
...
def stateful_multi_step_training_ppu(
    ...
    Returns:
        Tuple of (loss_history, final_model_state, final_opt_state, bn_stats_history)
    """
    ...
    def single_training_step_stateful(model_state, opt_state, x, y):
        ...
        # Compute loss and gradients with updated state
        loss, gradients = jax.value_and_grad(loss_function)(model_state)
        ...
        param_state = nnx.state(model_state, nnx.Param)
        updated_params = optax.apply_updates(param_state, updates)

        # Preserve batch norm statistics from the forward pass
        bn_stats = nnx.state(model_state, nnx.BatchStat)

        # Combine updated parameters with current batch norm stats
        new_model_state = nnx.State.merge(updated_params, bn_stats)
    ...
    def stateful_training_loop(
        ...
        def loop_body(step, carry):
            current_state, current_opt_state, loss_array, bn_means = carry
            ...
            # Track batch norm running mean for first layer (for monitoring)
            model_temp = nnx.merge(model_graphdef, new_state)
            current_bn_mean = jnp.mean(model_temp.bn1.mean)
            updated_bn_means = bn_means.at[step].set(current_bn_mean)

            # Record loss for this step
            updated_losses = loss_array.at[step].set(step_loss)

            return new_state, new_opt_state, updated_losses, updated_bn_means

        # Initialize tracking arrays
        loss_history = jnp.zeros(steps)
        bn_stats_history = jnp.zeros(steps)

        # Execute training loop
        final_state, final_opt_state, final_losses, final_bn_stats = jax.lax.fori_loop(
            0,
            steps,
            loop_body,
            (initial_state, initial_opt_state, loss_history, bn_stats_history),
        )

        return final_losses, final_state, final_opt_state, final_bn_stats
    ...
    result = mp.device("P0", fe_type="nnx")(stateful_training_loop)(
        initial_model_state, initial_opt_state, features, labels, num_steps
    )
...
def main():
    ...
    print(f"Model architecture: [8 → 16 (ReLU) → 2]")
    print(f"Total parameters: {param_count}")
    print(f"Focus: Simple multi-step training without state")
    ...
    print(f"Model architecture: [8 → 16 (BN+ReLU) → 16 (BN+ReLU) → 2]")
    print(f"Trainable parameters: {stateful_param_count}")
    print(f"Batch norm statistics: {bn_stat_count}")
    print(f"Focus: Stateful training with running statistics")
    ...
    simulator = mp.Simulator(cluster_config)
    print(f"Device configured: PPU (Plaintext Processing Unit)")
    print(f"Training will demonstrate multi-step learning on PPU")
    print(f"Data owner: node_0 (will fetch results from this party)")
    ...
    try:
        ...
        # Extract results from owner party (node_0)
        simple_training_data = extract_training_results(
            simple_result, simulator, "node_0"
        )

        if simple_training_data:
            plot_training_progress(
                simple_training_data["loss_history"], "Simple Model Training", "PPU"
            )
        else:
            print("✗ Failed to extract simple model training results")
    ...
    try:
        ...
        # Extract results from owner party (node_0)
        stateful_training_data = extract_stateful_training_results(
            stateful_result, simulator, "node_0"
        )

        if stateful_training_data:
            plot_training_progress(
                stateful_training_data["loss_history"], "Stateful Model Training", "PPU"
            )

            # Show batch norm statistics evolution
            bn_stats = stateful_training_data["bn_stats_history"]
            print(f"\nBatch Norm Statistics Evolution:")
            try:
                initial_mean = float(bn_stats[0]) if len(bn_stats) > 0 else 0.0
                final_mean = float(bn_stats[-1]) if len(bn_stats) > 0 else 0.0
                print(f"Initial running mean: {initial_mean:.6f}")
                print(f"Final running mean: {final_mean:.6f}")
                print(f"Statistics change: {final_mean - initial_mean:.6f}")
                print("✓ Batch normalization statistics tracked successfully")
            except Exception as e:
                print(f"Note: Batch norm statistics tracking encountered: {e}")
                print(
                    "✓ Model training completed successfully with batch normalization"
                )
        else:
            print("✗ Failed to extract stateful model training results")

    except Exception as e:
        print(f"✗ Stateful model training failed: {e}")
    ...
    if (
        "simple_training_data" in locals()
        and simple_training_data
        and "stateful_training_data" in locals()
        and stateful_training_data
    ):

        simple_loss = simple_training_data["loss_history"]
        stateful_loss = stateful_training_data["loss_history"]

        # Calculate improvements
        simple_improvement = (
            (float(simple_loss[0]) - float(simple_loss[-1]))
            / float(simple_loss[0])
            * 100
        )
    ...
    print("• Statistics evolution: Batch norm adapts to data distribution")
    print("• Owner party: Results must be fetched from data owner for extraction")
    print("=" * 80)
