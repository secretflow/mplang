...
class TestMPICommunication:
    ...
    def test_gather_m_basic(self):
        ...
        data = mplang.eval(sim, mpr.prandint, 0, 100)
        gathered = mplang.eval(sim, mpi.gather_m, (1 << 3) - 1, 0, data)
    ...
    def test_gather_m_different_root(self):
        ...
        # Test with root = 2
        data = mplang.eval(sim, mpr.prandint, 0, 100)
        gathered = mplang.eval(sim, mpi.gather_m, (1 << 4) - 1, 2, data)
        data, gathered = mplang.fetch(sim, (data, gathered))
    ...
    def test_gather_m_subset_parties(self):
        ...
        gathered = mplang.eval(sim, mpi.gather_m, src_mask, 0, data)
        data, gathered = mplang.fetch(sim, (data, gathered))
    ...
    def test_gather_m_two_parties(self):
        ...
        data = mplang.eval(sim, mpr.prandint, 0, 50)
        gathered = mplang.eval(sim, mpi.gather_m, (1 << 2) - 1, 1, data)
        data, gathered = mplang.fetch(sim, (data, gathered))
    ...
    def test_scatter_m_basic(self):
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: [10, 20, 30]))
        scattered = mplang.eval(sim, mpi.scatter_m, (1 << 3) - 1, 0, data)
    ...
    def test_scatter_m_different_root(self):
        ...
        data = mplang.eval(sim, simp.runAt(2, lambda: [100, 200, 300, 400]))
        scattered = mplang.eval(sim, mpi.scatter_m, (1 << 4) - 1, 2, data)
        data, scattered = mplang.fetch(sim, (data, scattered))
    ...
    def test_scatter_m_subset_parties(self):
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: [5, 15, 25]))
        # Scatter to parties 0, 2, 3 (mask = 1101 = 13)
        to_mask = (1 << 0) | (1 << 2) | (1 << 3)
        scattered = mplang.eval(sim, mpi.scatter_m, to_mask, 0, data)
        data, scattered = mplang.fetch(sim, (data, scattered))
    ...
    def test_scatter_m_large_data(self):
        ...
        data = mplang.eval(sim, simp.runAt(1, lambda: [9999, 8888]))
        scattered = mplang.eval(sim, mpi.scatter_m, (1 << 2) - 1, 1, data)
        data, scattered = mplang.fetch(sim, (data, scattered))
    ...
    def test_bcast_m_basic(self):
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: 42))
        # Broadcast from party 0 to all parties
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 3) - 1, 0, data)
    ...
    def test_bcast_m_different_root(self):
        ...
        data = mplang.eval(sim, simp.runAt(3, lambda: 123))
        # Broadcast from party 3 to all parties
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 4) - 1, 3, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
    ...
    def test_bcast_m_subset_parties(self):
        ...
        data = mplang.eval(sim, simp.runAt(1, lambda: 777))
        # Broadcast from party 1 to parties 0, 1, 3, 4 (mask = 11011 = 27)
        to_mask = (1 << 0) | (1 << 1) | (1 << 3) | (1 << 4)
        broadcasted = mplang.eval(sim, mpi.bcast_m, to_mask, 1, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
    ...
    def test_bcast_m_negative_value(self):
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: -456))
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 2) - 1, 0, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
    ...
    def test_bcast_m_zero_value(self):
        ...
        # Create zero data
        data = mplang.eval(sim, simp.runAt(2, lambda: 0))
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 3) - 1, 2, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
    ...
    def test_p2p_basic(self):
        ...
        # Create data at party 0
        data = mplang.eval(sim, simp.runAt(0, lambda: 99))
        # Send from party 0 to party 2
        sent = mplang.eval(sim, mpi.p2p, 0, 2, data)
    ...
    def test_p2p_different_pairs(self):
        ...
        data = mplang.eval(sim, simp.runAt(3, lambda: 555))
        sent = mplang.eval(sim, mpi.p2p, 3, 1, data)
        data, sent = mplang.fetch(sim, (data, sent))
    ...
    def test_p2p_multiple_transfers(self):
        ...
        data1 = mplang.eval(sim, simp.runAt(0, lambda: 100))
        sent1 = mplang.eval(sim, mpi.p2p, 0, 1, data1)

        # Second transfer: party 1 -> party 2
        data2 = mplang.eval(sim, simp.runAt(1, lambda: 200))
        sent2 = mplang.eval(sim, mpi.p2p, 1, 2, data2)
    ...
    def test_p2p_large_value(self):
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: large_value))
        result = mplang.eval(sim, mpi.p2p, 0, 2, data)
    ...
    def test_p2p_same_party(self):
        ...
        # Create data at party 0
        data = mplang.eval(sim, simp.runAt(0, lambda: 77))
        # Send from party 0 to party 0 (same party)
        sent = mplang.eval(sim, mpi.p2p, 0, 0, data)
        data, sent = mplang.fetch(sim, (data, sent))
    ...
    def test_p2p_same_party_different_parties(self):
        ...
        data = mplang.eval(sim, simp.runAt(2, lambda: 333))
        sent = mplang.eval(sim, mpi.p2p, 2, 2, data)
        data, sent = mplang.fetch(sim, (data, sent))
    ...
    def test_allgather_m_placeholder(self):
        ...
        with pytest.raises(NotImplementedError, match="Allgather not implemented"):
            mplang.eval(sim, mpi.allgather_m, (1 << 3) - 1, data)
    ...
    def test_edge_cases_and_error_handling(self):
        ...
        original_data = mplang.eval(single_party_sim, simp.runAt(0, lambda: 42))
        ...
        sent = mplang.eval(single_party_sim, mpi.p2p, 0, 0, original_data)
    ...
    def test_stress_with_many_parties(self):
        ...
        data = mplang.eval(sim, simp.runAt(4, lambda: 888))
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 8) - 1, 4, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
        ...
        data = mplang.eval(sim, simp.runAt(0, lambda: 111))
        # 0 -> 7
        sent = mplang.eval(sim, mpi.p2p, 0, 7, data)
        data, sent = mplang.fetch(sim, (data, sent))
    ...
    def test_data_types_variety(self):
        ...
        data = mplang.eval(sim, simp.runAt(1, lambda: 1234))
        broadcasted = mplang.eval(sim, mpi.bcast_m, (1 << 3) - 1, 1, data)
        data, broadcasted = mplang.fetch(sim, (data, broadcasted))
        assert broadcasted == [1234, 1234, 1234]

        # Test with very large numbers
        large_data = mplang.eval(sim, simp.runAt(0, lambda: 2**20))
        sent = mplang.eval(sim, mpi.p2p, 0, 2, large_data)
...
class TestPShfl:
    ...
    def test_pshfl_basic(self):
        ...
        src = mplang.eval(sim, mpr.prandint, 0, 100)
        key = mplang.eval(sim, mpr.ukey, 42)
        index = mplang.eval(sim, mpr.pperm, key)

        # shuffle data with range, nothing changed.
        shuffled = mplang.eval(sim, mpi.pshfl, src, index)
