...
def bit_count(mask: int | Mask) -> int:
    """Return the number of set bits in the mask.
    ...
    if isinstance(mask, Mask):
        ...
        # For backward compatibility, use assert like the original
        assert mask >= 0, f"Expect non-negative mask, got {mask}"
        return mask.bit_count()


def enum_mask(mask: int | Mask) -> Iterator[int]:
    """Enumerate the indices of set bits in the mask.
    ...
    if isinstance(mask, Mask):
        ...
        # For backward compatibility, use assert like the original
        assert isinstance(mask, int) and mask >= 0, f"Expect non-negative mask, got {mask}"
        return Mask(mask).enum()


def global_to_relative_rank(global_rank: Rank, mask: int | Mask) -> Rank:
...
def relative_to_global_rank(relative_rank: Rank, mask: int | Mask) -> Rank:
...
def is_rank_in(rank: Rank, mask: int | Mask) -> bool:
    """Check if a rank is in the party mask.
...
def ensure_rank_in(rank: Rank, mask: int | Mask) -> None:
...
def is_subset(subset_mask: int | Mask, superset_mask: int | Mask) -> bool:
...
def ensure_subset(subset_mask: int | Mask, superset_mask: int | Mask) -> None:
