...
# Add project root to sys.path to allow importing mplang
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../..")))
from mplang.v2.backends import load_backend
import mplang.v2.edsl as el
from mplang.v2.dialects import tensor, simp
from mplang.v2.backends.simp_simulator import SimpSimulator

# Import SGB implementation from local directory
...
def load_data(path_or_id: str, sample_size: int = None):
        ...
        if dataset_name.lower() in ["covertype", "covtype", "1597", "150"]:
            ...
            try:
                ...
                if dataset_name.isdigit():
                    bunch = fetch_openml(data_id=int(dataset_name), as_frame=True, parser='auto')
                else:
    ...
    if len(unique_y) > 2:
        print(f"Dataset has {len(unique_y)} classes. Binarizing (Class {unique_y[0]} vs Rest)...")
...
def train_eval_sgb(X_train, X_test, y_train, y_test, params, pp_feature_ratio=0.5):
    """
    ...
    def to_np(d): return d.values.astype(np.float32) if hasattr(d, 'values') else d.astype(np.float32)
    ...
    with el.Tracer() as tracer:
        # Define MPC inputs via pcall_static (placing data on specific parties)
        d_ap = simp.pcall_static((0,), lambda: tensor.constant(X_ap_train))
        d_y = simp.pcall_static((0,), lambda: tensor.constant(y_train))
        d_pp = simp.pcall_static((1,), lambda: tensor.constant(X_pp_train))
        ...
        # Predict
        d_ap_test = simp.pcall_static((0,), lambda: tensor.constant(X_ap_test))
        d_pp_test = simp.pcall_static((1,), lambda: tensor.constant(X_pp_test))
        y_prob = model.predict([d_ap_test, d_pp_test], n_samples=len(y_test))
        
        # 'finalize' marks the tensor we want to retrieve from the simulation
        tracer.finalize(y_prob)
        
    trace_time = time.perf_counter() - start_time
    print(f"Graph tracing finished in {trace_time:.2f}s")
    
    # --- 2. Execute Graph ---
    print("\nExecuting graph (Simulating 2 parties)...")
    
    host = SimpSimulator(world_size=2)
    # Use thread pool to parallelize local FHE operations
    import concurrent.futures
    max_workers = os.cpu_count() or 4
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
    async_ops = {"bfv.add", "bfv.mul", "bfv.rotate", "bfv.batch_encode", 
                 "bfv.relinearize", "bfv.encrypt", "bfv.decrypt"}
    host.set_worker_executor(executor, async_ops)
    
    exec_start = time.perf_counter()
    result = host.evaluate_graph(tracer.graph, [])
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, help="Path to CSV or 'openml:<name>'")
    ...
    # 2. Run SecureBoost
    sgb_auc, sgb_time = train_eval_sgb(X_train, X_test, y_train, y_test, params)
